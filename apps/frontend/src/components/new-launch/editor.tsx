      ...
      {!canEdit && !isCreateSet && (
        <>
          ...
          >
            ...
            <div>
              <div className="text-white rounded-[8px] h-[44px] px-[20px] bg-[#1F3A5F] cursor-pointer flex justify-center items-center">
                        ...
                        {!!internal && !existingData?.integration && (
                          ...
                          >
                            <div className="flex gap-[6px] items-center">
                              <div className="w-[8px] h-[8px] rounded-full bg-[#FF6B5B]" />
  ...
  const { getRootProps, isDragActive } = useDropzone({
    onDrop: (files) => {
      if (loading) {
        toaster.show(
          t(
            'upload_in_progress_retry_later',
            'アップロード中です。完了してからもう一度お試しください。'
          ),
          'warning'
        );
        return;
