...
type PostingTextCapability = { feature: 'posting_text'; status: 'configured' | 'disabled' | 'not_configured'; canStart: boolean; verification: 'not_run'; replyQuotaShared: false };
...
export function usePostingTextCapability() {
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR('toybaco-posting-text-capability', load, {
...
export const ToybacoAssistedTextarea = ({ autosuggestionsConfig, disableBranding: _branding, ...props }: AssistedProps) => {
  const { available, checking, error, retry } = usePostingTextCapability();
  ...
  return (
    <div data-toybaco-assisted-textarea="">
      ...
      <div data-toybaco-assistance-actions="">
