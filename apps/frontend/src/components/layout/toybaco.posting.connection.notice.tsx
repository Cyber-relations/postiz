...
export function ToybacoPostingConnectionNotice({ placement = 'shell' }: { placement?: 'shell' | 'composer' }) {
  ...
  return (
    <div data-toybaco-posting-notice={placement} role={posting.renewing ? 'status' : 'alert'} aria-live={posting.renewing ? 'polite' : undefined}
      ...
      {!posting.renewing && posting.appOrigin && posting.context && (
      ...
      {!posting.renewing && <button type="button" disabled={verifying} className="min-h-[44px] rounded-lg border border-newTextColor/20 px-4 py-2 font-semibold disabled:opacity-50"
