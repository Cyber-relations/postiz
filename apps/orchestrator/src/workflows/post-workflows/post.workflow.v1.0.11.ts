...
const {
  ...
  getPost,
  inAppNotification,
  toybacoChangeStateV110,
  toybacoUpdatePostV110,
  toybacoGetWorkflowReadinessV110,
  toybacoCompleteCommentV110,
  sendWebhooks,
  isCommentable,
...
export async function postWorkflowV111({
  ...
  // toybaco_approval_flow_v5: only V110 uses activities bound to an exact publish token.
  let rootFinalStep: 'MAIN' | 'FINALIZE' = 'MAIN';
  const changeState = (id: string, state: State, err?: any, body?: any) =>
    toybacoChangeStateV110(
      organizationId,
      id,
      state,
  ...
  const updatePost = (id: string, providerPostId: string, releaseURL: string) =>
    toybacoUpdatePostV110(
      organizationId,
      id,
      providerPostId,
      releaseURL,
      expectedPublishMarker,
      id === postId ? rootFinalStep : 'COMMENT',
    );
  ...
  // signalWithStart may start a workflow before the database acknowledgement.
  // Run read-only activities until the exact marker is READY, and exit without
  // side effects if the marker advances to another generation.
  while (true) {
    const readiness = await toybacoGetWorkflowReadinessV110(
      organizationId,
      postId,
      expectedPublishMarker,
    );
    if (readiness === 'STALE') return;
  ...
  const { toybacoCheckPostStatusV111 } = proxyCheckTaskQueue(taskQueue);
  const checkPostStatus = (integration: Integration, pendingData: any) =>
    toybacoCheckPostStatusV111(
      integration,
      pendingData,
      postId,
      expectedPublishMarker,
  ...
  const { toybacoPostCommentV110 } = proxyCommentTaskQueue(taskQueue);
  const postComment = (
  ...
  const {
    toybacoPostSocialV110,
    toybacoFinalizePostV111,
    toybacoPrepareRepeatV110,
  } = proxyMutationTaskQueue(taskQueue);
  const postSocialPending = (_integration: Integration, _posts: Post[]) =>
    toybacoPostSocialV110(
      postId,
      organizationId,
      expectedPublishMarker,
      postNow ? State.PUBLISHED : State.QUEUE,
    );
  const finalizePost = async (
    ...
    const result = await toybacoFinalizePostV111(
      integration,
      pendingData,
      postId,
      expectedPublishMarker,
  ...
  // official 0f1647f7: scheduled waiting is not subtracted from the repeat interval.
  ...
  const handleActivityError = async (
    ...
    if (cause?.type === 'refresh_token') {
      const integrationForRefresh = getIntegration
        ? await getIntegration()
        : post.integration;
      const refresh = await refreshTokenWithCause(
        integrationForRefresh,
        cause.message || '',
      );
      ...
      if (!getIntegration) {
        integrationForRefresh.token = refresh.accessToken;
      }
      ...
      try {
        ...
        if (result.status === 'ready') {
          result = await finalizePost(
            post.integration,
            result.pendingData,
            result.toybacoContinuation,
          );
          if (result.status !== 'completed') check = -1;
        }
      ...
      try {
        ...
        if (postsResults[i].status === 'pending') {
          ...
          try {
            ...
            // would retry the post and duplicate it. Best-effort child/root outcome.
            try {
              if (i > 0) {
                await completeComment(postsList[i].id, 'UNCONFIRMED', err);
              } else {
                await markUnconfirmed(err);
              }
            } catch (e) {
        ...
        // Finalize comment failures per child; never downgrade a published root to ERROR.
        if (i > 0) {
          const unconfirmed =
            posted ||
  ...
  while (list.length > 0) {
    ...
    // process repeat post in a distinct token-bound V110 workflow.
    if (todo.type === 'repeat-post') {
      const next = await toybacoPrepareRepeatV110(
        organizationId,
        postId,
        expectedPublishMarker,
      );
        ...
        args: [
          {
            ...
            organizationId,
            postNow: true,
            expectedPublishMarker: next.marker,
          },
        ],
